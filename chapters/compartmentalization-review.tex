...
\subsection{Intel Memory Protetion Keys (MPK)}
% Context and introduction of MPK
While the concept of protection keys to regions of memory predates 
Intel's MPK extension, previous implementations in PA-RISC,
Itanium and POWER-6 rely on supervisor managed protection keys whereas
MPK enables the user to modify permission keys stored in a dedicated
PKRU register.
MPK uses bits of the page table entry to assign each page a 4-bit color,
and uses permission bits in the PKRU register to control permissions to
each page color.
...
% Context and introduction of XXXXX

% Explain how XXXXX allow compartmentalization
% Isolation part
% Communication part

% Explain how XXXXX compartmentalize a browser
