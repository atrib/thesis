...
\begin{figure}[]
  \centering
  \includegraphics[width=0.9\linewidth]{media/midas/midas_speculative_forward.pdf}
  \caption[Illustration of a theoretical speculative leakage attack against Midas' protection.]
          {Illustration of a theoretical speculative leakage attack against Midas' protection.
           The attacker uses the core's shared load-store queue (LSQ) to forward 
           the value from the attacker's illegal speculative write to a protected page
           to the victim syscall's read.
           The incorrect speculation will be rolled back eventually, but can be used to
           transiently leak a kernel secret into a side-channel.
          }
  \label{fig:midas:midas_spec_forward}
\end{figure}

For this attack, we assume that the attacker and victim run on two logical
hardware threads running on a single physical core on an SMT processor.
The steps of the attack are described below, and illustrated in
\autoref{fig:midas:midas_spec_forward}.
\begin{enumerate}
  ...
  \item Both the attacker and victim continue on an architecturally legal
        execution path.
        The attacker reads the victim's secret from a side-channel.
\end{enumerate}
...
Each page table entry for a user page in different address spaces
...
For example, \Code{process_vm_writev} can be used to modify 
...
In this section, we empirically verify \midas' ability to mitigate
...
a load generator (\Code{bombardier}) with 100 concurrent
