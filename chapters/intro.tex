\atri{Tackle general theme of repitition in this intro}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         at all.
% \end{itemize}
\mat{why should they change if they work?}
\atri{The following sentences address exactly that.}
...
These static analysis techniques have progressively generalized,
...
change potentially introduces a new double-fetch bug.
...
A systematic mitigation for double-fetch bugs must prohibit all 
...
The kernel's system call interface is crucial for protecting the system, as
the kernel is privileged and can access and control the system's resources.
Compromising the kernel through system calls can allow attackers to bypass
inter-process isolation, compromise applications and data for other users of
the system, and arbitrarily control external interfaces such as network cards
exposing the potential to attack other connected systems.
Intra-process trust boundaries have emerged more recently as 
security-critical interfaces as modern systems increasingly incorporate 
untrusted components and get connected to the internet.
Bugs in network connected applications with vulnerable code components
represents the first step towards remotely compromising systems.
Attacks can leverage bugs in a component to gain access to the entire
process, and can then leverage that capability to attack the kernel
