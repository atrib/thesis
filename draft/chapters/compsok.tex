...
proposals for compartmentalization mechanisms.
However, mechanisms\footnotemark differ in the threat models they are designed
to protect against, their performance characteristics, supported use cases,
and their compatibility with existing or upcoming system architectures.
...
compartmentalization mechanisms and compare their 
...
These examples will allow us to highlight the necessity of particular
security or performance properties for mechanisms, and to thereby
compare the mechanisms described later.
Practically, applications may be composed of compartments having any
mix of the following trust relations.
...
\begin{figure}
      \centering
      \includegraphics[width=0.75\linewidth]{media/compreview/hierarchy_trust.pdf}
      \caption[Illustrating hierarchical trust between application compartments.]
              {Example of sandboxing applications, illustrating a hierarchy of
              trust between compartments.
              Trust relations are generally transitive, and nested sandboxes rely
              indirectly on the sandboxing engine.}
      \label{fig:compreview:asdf}
\end{figure}
...
by Schrammel~\textit{et.~al.}~\cite{schrammel2022jenny}).
...
\atri{TODO: Integrate and phrase this section around the new example.}
Compartments within a hierarchical trust-based system can also contain 
mutually distrusting compartments.
For example, two sandboxes managed by the same engine might distrust each
other.
However, they can rely on a common root of trust, like the sandbox engine, to 
mediate operations between them.
Calls between sandboxes, for example, pass via the Engine.
The Engine, therefore, can implement isolation between the sandboxes.
This model remains separate from the model of mutual distrust described
in the next section.
...
Compartmentalization should limit which compartments are allowed to call
which other compartments, as an application of least privilege. 
...
For example, the stack pointer register could point to an attacker-crafted
malicious stack.
...
Memory shared between all stages removes some compartmentalization benefits, 
as a corrupted stage can also corrupt data being processed by other stages.
